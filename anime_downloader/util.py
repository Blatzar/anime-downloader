...
def search(query, provider, val=None, season_info=None, ratio=50):
    ...
    while True:
        if val is None:
            val = click.prompt('Enter the anime no{}:'. format(' (0 to switch provider)' * (season_info is not None)),
...
def parse_episode_range(max_range, episode_range):
    ...
    if episode_range.endswith(':'):
        length = max_range if isinstance(max_range, int) else len(max_range)
